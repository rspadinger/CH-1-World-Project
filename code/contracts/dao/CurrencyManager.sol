...
contract CurrencyManager is ICurrencyManager, AccessControl {
  ...
  //@note use of EnumerableSet.AddressSet
  ...
  function addCurrency(address currency) external override onlyRole(ADMIN_ROLE) {
    if (currency == address(0)) revert CurrencyManagerError("Cannot be null address");
    if (_whitelistedCurrencies.contains(currency)) revert CurrencyManagerError("Already whitelisted");
  ...
  function removeCurrency(address currency) external override onlyRole(ADMIN_ROLE) {
    if (!_whitelistedCurrencies.contains(currency)) revert CurrencyManagerError("Not whitelisted");
  ...
  function viewWhitelistedCurrencies(
    ...
    for (uint256 i = 0; i < length; i++) {
      whitelistedCurrencies[i] = _whitelistedCurrencies.at(cursor + i); //@audit-ok make sure at starts at 0 => yes
    }

    return (whitelistedCurrencies, cursor + length); //@audit-ok !!! low => should be: length - cursor => mayb, this should be next cursor pos
  }
}
