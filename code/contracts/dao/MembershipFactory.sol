...
contract MembershipFactory is AccessControl, NativeMetaTransaction {
  ...
  function createNewDAOMembership(DAOInputConfig calldata daoConfig, TierConfig[] calldata tierConfigs) external returns (address) {
    //console.log(daoConfig.currency, daoConfig.noOfTiers, daoConfig.ensname);
    ...
    //@audit-ok what if creator assigns lower price for higher tier => 2 lower tokens are burned for 1 higher minted
    //@audit-ok what if daoConfig.noOfTiers == tierConfigs.length, butsome tiers are empty => 0 values - eg: amount
    ...
    //this is the proxy for the newly created DAO
  ...
  function updateDAOMembership(
    ...
    //@audit-ok we potentially delete higher tiers => what about users who already paid
    delete dao.tiers;
    for (uint256 i = 0; i < tierConfigs.length; i++) {
      dao.tiers.push(tierConfigs[i]);
  ...
  //tierIndex : 0 - 6 => 0 is the most expensive
  ...
  function upgradeTier(address daoMembershipAddress, uint256 fromTierIndex) external {
    ...
    //0 - 6 => user provides final tier: 6 => 7 >= 7
    require(daos[daoMembershipAddress].noOfTiers >= fromTierIndex + 1, "No higher tier available.");

    //burn 2 tokens from current tier == 1 token of next tier
    IMembershipERC1155(daoMembershipAddress).burn(_msgSender(), fromTierIndex, 2); //@audit-ok why do we burn 2 tokens
    IMembershipERC1155(daoMembershipAddress).mint(_msgSender(), fromTierIndex - 1, 1); //@audit-ok shouldnt this be +1 => above we check: noOfTiers >= fromTierIndex + 1
  ...
  //@audit-ok - risky? => why not use UUPS
