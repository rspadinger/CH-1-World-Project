...
contract MembershipERC1155 is ERC1155Upgradeable, AccessControlUpgradeable, IMembershipERC1155 {
  ...
  function mint(address to, uint256 tokenId, uint256 amount) external override onlyRole(OWP_FACTORY_ROLE) {
    //indicates that tier 0 is the highest => should be the lowest?
    //what if we have more than 7 tiers : 64,32,16,8,4,2,1, 0... => from level 8 onwards, supply does not increase any longer
    //@audit-ok 7 tiers => tier0 is highest (most rewards)
  ...
  function burnBatch(address from) public onlyRole(OWP_FACTORY_ROLE) {
    for (uint256 i = 0; i < 7; ++i) {
      //@audit-ok magic number - we assume there are up to 7 tiers : 0-6
  ...
  function uri(uint256 tokenId) public view virtual override returns (string memory) {
    return
      string(
        abi.encodePacked(super.uri(tokenId), Strings.toHexString(uint256(uint160(address(this))), 20), "/", Strings.toString(tokenId))
      );
  }
  ...
  //called on token transfer & on claimProfit()
  ...
  function getUnsaved(address account) internal view returns (uint256 profit) {
    ...
    // (#Token * ACC / TotSupp) * PartUserSupply / Acc => #Token * PartUserSupply / TotSupp
    //@audit-ok !!! could a user game this depending on when he enters => for a new user, lastProfit[account] is always 0 => unsaved is max =>
    //for a new user, lastProfit[account] needs to be set to totalProfit : lastProfit[account] = totalProfit;
    //if(lastProfit[account] == 0) lastProfit[account] = totalProfit;
    //console.log("account: ", totalProfit, lastProfit[account], shareOf(account));
